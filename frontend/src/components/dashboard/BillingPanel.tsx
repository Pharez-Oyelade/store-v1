...
const PLANS = [
  {
    ...
    features: [
      "5 Active Products",
      "5 Orders & Demands / month",
      "10 Customer Measurement Cards",
  ...
  },
  {
    ...
    features: [
      "50 Active Products",
      "25 Orders & Demands / month",
      "100 Customer Measurement Profiles",
      "Bespoke Demand Board & Workflow",
      "Up to 3 Suppliers & Fabric Debt",
      "7-Day Sales Snapshot Analytics",
  ...
  },
  {
    ...
    features: [
      "200 Active Products",
      "500 Orders & Demands / month",
      "Unlimited Customer Profiles",
      "3 Team Seats (Manager, Tailor, Sales)",
      "Unlimited Suppliers & Fabric Debt",
      "12-Month Revenue Analytics & Slow Movers",
      "Custom WhatsApp Message Templates",
      "Orders CSV Data Export",
  ...
  },
  {
    ...
    features: [
      ...
      "10 Team Seats & Staff Delegation",
      "Full Atelier BI Command Center",
      "Bespoke vs RTW Revenue Split",
      "Tailor Turnaround & Productivity",
      "Gross Margin & Unit Economics",
      "Full CSV Export Suite",
...
export function BillingPanel() {
  ...
  const handleUpgrade = async (planId: string) => {
    ...
    try {
      const data = (await initUpgradeMutation.mutateAsync({
        plan: planId,
      })) as any;

      if (data.isFree) {
        toast.success(
          `Successfully changed to ${PLANS.find((p) => p.id === planId)?.name} plan`,
        );
      ...
      initializePayment({
        ...
        amount: parseInt(
          PLANS.find((p) => p.id === planId)!.price.replace(/\D/g, "") + "00",
          10,
        ),
        metadata: {
          vendorId: vendor?._id,
  ...
  const isProcessing =
    initUpgradeMutation.isPending || verifyUpgradeMutation.isPending;
            ...
            >
              {isCurrent && (
                <div className="absolute -top-3 right-6">
                  <span className="bg-brand-400 text-white text-xs px-3 py-1 rounded-full font-medium shadow-sm">
                    Current Plan
                  </span>
                </div>
              )}

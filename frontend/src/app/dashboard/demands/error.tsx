...
export default function DemandsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Bespoke Demands section error:", error);
  }, [error]);
  ...
  return (
    <div className="mx-auto max-w-xl py-12 text-center space-y-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-1">
        <h2 className="text-lg font-bold text-gray-900">Failed to load bespoke demands</h2>
        <p className="text-xs text-gray-600 max-w-md mx-auto leading-relaxed">
          {error?.message || "An unexpected error occurred while loading bespoke tailoring demands."}
        </p>
      </div>

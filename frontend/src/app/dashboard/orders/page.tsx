import OrdersPage from "@/app/(landing)/components/OrdersPage";
...
//   const orders = useOrders({ page: 1, limit: 50, status: initialStatus ?? undefined });
...
//         action={
//           <Link href="/dashboard/orders/new" className="inline-flex h-9 items-center gap-2 rounded-md bg-brand-700 px-3 text-sm font-medium text-white hover:bg-brand-800">
...
//                   order={order}
//                   onDelete={() => {
//                     if (confirm(`Delete order for ${order.customerSnapshot.name}?`)) deleteOrder.mutate(order._id);
//                   }}
//                 />
//               ))}
//             </tbody>
...
//         <Link href={`/dashboard/orders/${order._id}`} className="font-medium text-gray-950">{order.customerSnapshot.name}</Link>
...
//             className="h-8 w-32"
//             value={order.status}
//             onChange={(event) => updateOrder.mutate({ status: event.target.value as OrderStatus })}
//           >
...
//           </a>
//           <button onClick={onDelete} className="inline-flex size-8 items-center justify-center rounded-md border border-gray-200 text-error-600 hover:bg-error-50" aria-label="Delete order">
